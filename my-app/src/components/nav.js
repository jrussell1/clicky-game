...
class Nav extends Component {
    ...
    renderMessage(correct, gameWon, clear = false) {
        ...
        message = gameWon ? 'You won!!' : (correct ? 'You guessed right!' : 'You guessed wrong')
            ...
            <nav className="pinned">
                <div>
                    <ul className="center navList">
                        <li clasName="logo">Clicky Game</li>
                        {this.renderMessage(this.props.correct, this.props.gameWon)}
                        <li> Score: {this.props.score} | High Schore: {this.props.highScore}</li>
                    </ul>
                </div>
            </nav>
        )
